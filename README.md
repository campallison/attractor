...
| Behavioral Detection | Read-loop detection, nudge injection, empty-output detection | Detects 5+ consecutive read-only rounds and injects a course-correction nudge (1 per stage); warns when a stage produces no deliverable files (suppressible with `allow_empty_output=true`) |
