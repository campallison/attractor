...
- **Prompt Caching:** `WithPromptCaching()` enables Anthropic's `cache_control` mechanism for system and user messages. Cached prefixes are billed at ~10% of normal input cost. The `Usage` struct tracks `CacheReadTokens` and `CacheCreationTokens`. Cache breakpoints are placed strategically (system message, first user message, last user messages) and capped at Anthropic's 4-block limit via `selectCacheBreakpoints`.
